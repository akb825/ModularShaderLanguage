...
* `SPIRV_VERSION`: (SPIR-V target only) The SPIR-V version number in the format 0xABBCC, where A is the major version, B is the minor version, and C is the patch version. For example, version 1.4 would be 0x10400.
...
* `METAL_VERSION`: (macOS or iOS Metal target only) The Metal version number, represented as 100x the major version and 1x the minor version. For example, version 2.1 would be 201.
